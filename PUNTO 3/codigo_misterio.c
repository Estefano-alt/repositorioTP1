...
void f_mitadNumero(int *p) {
    *p = *p / 2;
}

void f_sumarDigitoscomponentesAlValorOriginal(int *p) {
    int temp = *p;
    int suma = 0;
    while (temp > 0) {
        suma = suma + (temp % 10);
...
void procesar_enigma(int *valor_referencia) {
    ...
    f_mitadNumero(valor_referencia);
    f_sumarDigitoscomponentesAlValorOriginal(valor_referencia);
}
